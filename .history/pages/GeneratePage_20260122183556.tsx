...
import { RefreshCcw, LayoutGrid, Timer, Trophy, BarChart2, Volume2, Cpu, Users } from 'lucide-react';
...
const GeneratePage: React.FC<{ members: Member[] }> = ({ members }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (mode === GenerationMode.AI_SMART) {
      generatedTeams = await generateSmartTeams(members, numTeams);
    } 
    ...
    if (generatedTeams.length === 0) {
      ...
      if (mode === GenerationMode.BALANCED) {
        ...
        const captains = shuffled.filter(m => m.role === 'Captain');
        const others = shuffled.filter(m => m.role !== 'Captain');
        generatedTeams.forEach((team, i) => { if (captains[i]) team.members.push(captains[i]); });
  ...
  setTimeout(() => {
    if (members.length<2){window.location.hash = "#members";
      toast.error("Please add more members to generate teams at least 2.");
      alert("Please add more members to generate teams at least 2.");
    };
  }, 100);
  return (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { id: GenerationMode.AI_SMART, label: t.generate.modes.ai, icon: <Cpu size={14} />, emoji: '🧠' },
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setMode(item.id)}
                    className={`p-4 rounded-xl border transition-all flex flex-col gap-2 group relative overflow-hidden ${lang === 'ar' ? 'text-right' : 'text-left'} ${
                  ...
                  >
                    ...
                    <span className={`text-[11px] font-bold transition-colors ${mode === item.id ? 'text-white' : 'text-gray-400'}`}>{item.emoji} {item.label}</span>
